...
2. **DB/HTTPS keepalive** (`jdi-portal/src/instrumentation.ts`): 2분 주기로 pg 풀과 Supabase 경로를 데워 콜드 스타트(유휴 후 첫 요청 3~7초)를 방지합니다. `setInterval`/keepalive와 pg 풀 설정(`min:1`, `keepAlive:true`, `idleTimeoutMillis: 10*60_000` — `src/lib/db/postgres.ts`)을 유지합니다. 운영(Cloud Run)은 CPU 요청기반 과금이라 요청 사이에 CPU 가 멈추므로, **Cloud Scheduler 작업 `jdi-portal-keepalive`(1분마다 `/api/health`)가 이 타이머를 깨웁니다** — 지우면 안 됩니다(`jdi-portal/docs/operations/cloud-run-seoul.md`).
