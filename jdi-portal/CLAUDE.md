...
- 배포: **GCP Cloud Run 서울(`asia-northeast3`)** — 프로젝트 `jdi-portal-seoul`, 서비스 `jdi-portal`. **`master` 병합 시 Cloud Build 트리거 `deploy-master-to-seoul`(global 리전)이 자동 배포**(2026-07-30부터). 수동 배포는 루트에서 `gcloud builds submit --config cloudbuild.yaml`(master 먼저 합칠 것). `jdiportal.com` 은 Cloudflare Worker 가 전달. 절차·롤백·비용은 `docs/operations/cloud-run-seoul.md`. (2026-07-30 Railway 싱가포르에서 이전, Railway 배포는 중지)
