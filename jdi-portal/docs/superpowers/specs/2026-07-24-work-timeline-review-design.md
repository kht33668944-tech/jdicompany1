...
- 예상 마이그레이션 번호: `108_work_timeline_reviews.sql` (테이블·인덱스·RLS·RPC·트리거), 재촉은 같은 파일 또는 `109`로 분리 가능
...
> **후속 설계 있음 — 🔄 방향 전환 v3**: 작성자가 검토자를 지정해 "확인해 주세요"라고 요청하는 반대 방향과
> 동료 간 상호 검토를 추가한다. 이 문서의 흐름(관리자 → 작성자 지시형)은 그대로 남는다.
> → `2026-08-04-work-timeline-review-request-direction-design.md`
...
## 🔄 방향 전환 v2 (2026-07-24, 1차 구현·테스트 후 사용자 피드백)

1차 버전은 "검토 요청 시 별도 할일을 자동 생성 → 작성자가 그 할일을 완료 → 트리거로 검토대기"였다.
브라우저 테스트 후, 사용자가 **"할일과 업무보고는 별개다. 보완은 업무보고 검토 칸에서 다 처리해야 한다"**고 방향을 바꿨다.
아래 표가 v2 결정이며, **v2가 v1의 해당 항목을 대체**한다.

| 항목 | v2 결정 |
|---|---|
| 자동 할일 | **생성하지 않음.** `tasks` 연결(`review_id`)·완료 감지 트리거·역링크 제거 |
| 보완 처리 위치 | **업무보고 상세의 검토 칸에서만.** 별도 할일 화면 없음 |
...
드롭(트리거·`sync_review_on_task_status`·`tasks.review_id`·`tasks_review_unique`·`reviews.task_id`) + 첨부 테이블/RLS 추가 + RPC 4종 `CREATE OR REPLACE`(할일 로직 제거, `submit_...` 신설).

**v2에서 유지**: 권한(관리자+작성자), 요청자=검토자, 진행 중 1건, 승인/반려 루프, 이력 타임라인, 알림 3종, 출근 인박스, 재촉, RLS 열람 제한.
...
## 1) DB — 마이그레이션 `108_work_timeline_reviews.sql`
