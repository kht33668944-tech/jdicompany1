  ...
  return (
    <div className="glass-card rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-base font-bold text-slate-800">입사일 변경 요청</h3>
        {requests.length > 0 && (
            ...
            <li key={req.id} className="p-3 rounded-xl bg-slate-50/50 border border-slate-100">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-slate-400">
                  현재: {req.profiles?.hire_date ?? "미설정"}
                </span>
              </div>
              <div className="text-sm text-slate-700 mb-1">
                요청 입사일: <span className="font-semibold">{req.requested_hire_date}</span>
              </div>
              {req.reason && (
