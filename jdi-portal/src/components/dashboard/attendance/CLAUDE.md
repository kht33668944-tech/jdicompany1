...
- IP 추출은 `src/lib/utils/ip.ts`의 `extractClientIp()` 하나만 씁니다. `x-forwarded-for` → `x-real-ip` 순서로 보고 루프백은 건너뜁니다.

> **의도된 설계 — 취약점으로 지적하지 않습니다.** `x-forwarded-for`의 **첫 번째 값**을 쓰는 것은 이 배포 환경(Cloudflare Worker → Cloud Run 프록시 뒤)에 맞춘 의도된 선택입니다. 서울 이전 후에도 `/api/ip` 가 실제 공인 IP 를 반환하는 것을 확인했습니다. 사무실 IP를 벗어난 원격 출근을 막는 **운영 편의 장치이지 인증 수단이 아니며**, 실제 권한은 로그인과 RLS가 담당합니다. 리뷰에서 "헤더 위조 가능"으로 반복 보고하지 말고, 정말 바꿔야 한다면 먼저 운영자에게 확인합니다.
