...
- 이 브랜치의 최신 파일은 `117_activity_log.sql`이고, **다음 번호는 118**입니다.
- 기존 마이그레이션을 수정하기보다 새 마이그레이션을 추가합니다.
- 롤백이 필요한 변경은 되돌리는 SQL을 함께 생각합니다.

### 번호를 정하기 전에 반드시 확인

`111`~`116`은 이 브랜치에 파일이 없습니다. 병합되지 않은 다른 작업 브랜치가 그 번호를 **이미 운영 DB에 적용**했기 때문입니다.
...
- 새 번호는 **로컬 파일 최댓값이 아니라 원격 적용 최댓값 다음**으로 잡습니다.
- push 후에는 반드시 `migration list`로 새 번호가 Remote에 찍혔는지 확인합니다.
